...
function sendEmail(){
	...
	console.log(email+']n'+name+'\n'+contact+'\n'+message);
	console.log('pasok sa sendmail')
	$.ajax({
		type: 'post',
		url: 'php/handler.php',
		data: {sendMail:'sendMail', name:name, contact:contact, message:message, email:email},
		success: function(e){
			console.log(e)
			if(e.match("true")){
				Materialize.toast('Message successfully sent', 4000);
				$('#email').val('');
				$('#name').val('');
				$('#contact').val('');
				$('#txtMessage').val('');
				...
				$('.loading').addClass('hide');
				Materialize.toast('Message not successfully sent \n slow network...', 4000);
			}
		}
	})
}
...
function responsiveHomeCards(){
	...
	$('.index .cards').find('.col').each(function(){
		if(height < $(this).find('.card-content').find('h5').height()){
			height = $(this).find('.card-content').find('h5').height();
			
			h3height = parseInt($(this).find('.card-content').find('h3').height());
			console.log(height);
		}
		if(cardImgHeight < $(this).find('.card-image').height()){
			cardImgHeight = $(this).find('.card-image').height();
			console.log(cardImgHeight);
		}
	});
	
	$('.index .cards').find('.col').each(function(){
		$(this).find('.card-content').find('h5').css('height', height+'px');
		$(this).find('.card-content').find('h3').css('height', h3height+'px');
		$(this).find('.card-image').css('height', cardImgHeight+'px');
	});
	
	height=0;
	h3height =0;
	cardImgHeight =0;
	$('.take-action .cards').find('.col').each(function(){
		
		if(height < $(this).find('.card-content').find('h6').height()){
			height = $(this).find('.card-content').find('h6').height();
			h3height = parseInt($(this).find('.card-content').find('h3').height());
		}
	...
	$('.take-action .cards').find('.col').each(function(){
		...
		$(this).find('.card-content').find('h6').css('height', height+'px');
		console.log(height +' '+ h3height)
		$(this).find('.card-content').find('h3').css('height', h3height+'px');
		$(this).find('.card-image').css('height', cardImgHeight+'px');
